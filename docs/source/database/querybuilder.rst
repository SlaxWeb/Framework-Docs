...
method can also throw the following two exceptions:

* **\\SlaxWeb\\Database\\Exception\\QueryException** - error occurred with the query
* **\\SlaxWeb\\Database\\Exception\\NoDataException** - no data could be obtained
  from the database or it yielded an invalid result set
